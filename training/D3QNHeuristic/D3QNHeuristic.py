...
from utils.modules.SMActorModule import ESMGreedyModule, ExploState2GreedyModule
...
def main(args):
    ...
    path = "training/D3QNHeuristic/Data/" + str(currentTime)
    os.mkdir(path)
    path += "/"
    ...
    for changedValue, runParameters, endRepetition in argGenerator.argsGenerate(plotArg=True):
        ...
        smBreakParam = parameters["State Machine Parameters"]["epsilon"]
        stateMachineEps = parameters["State Machine Parameters"]["stateMachineEps"]
        #State Exploration Parameters
        alphaOubli = parameters["State Exploration Parameters"]["alphaOubli"]
        timeStepInit = parameters["State Exploration Parameters"]["timeStepInit"]
        timeStepMax = parameters["State Exploration Parameters"]["timeStepMax"]
        valeurMax = parameters["State Exploration Parameters"]["valeurMax"]
        ...
        recorder.addData("actionSpec", actionSpec)
                ...
                # SM Instantiation
        smPolitic = politicInstantiationTensordict(smName, smBreakParam, observationNorm)
        ...
        explorationState2 = ExploState2GreedyModule(
            spec=actionSpec,
            recorder=recorder,
            workerNumber=workerNumber,
            windowAverage=totalTimeStep // 500,
            alphaOubli=alphaOubli,
            timeStepInit=timeStepInit,
            timeStepMax=timeStepMax,
            rolloutBatchTimeStep=rolloutBatchTimeStep,
            valeurMax=valeurMax,
        )

        actorExplore = TensorDictSequential(actor, exploration_module, explorationState2)
        ...
        for i, batch in enumerate(dataCollector):
            ...
            recorder.addData("zeta", copy.deepcopy(actorExplore[2].epsState))
        ...
        for i in range(nbState):
            ax.fill_between(X, lastProportion, lastProportion + stateProportionWindowed[i], label="State " + str(i))
        ...
        lastProportion = torch.zeros(len(stateProportionWindowed[0]))
        for i in range(nbState):
            ax.fill_between(X, lastProportion * episodeLengthwWindowed, (lastProportion + stateProportionWindowed[i]) * episodeLengthwWindowed, label="State " + str(i))
            lastProportion +=  stateProportionWindowed[i]
        ...
        plt.plot(X, recorder.dataRecorded["zeta"])
        plt.xlabel("Environment steps")
        plt.ylabel("Zeta value")
        plt.savefig(recorder.savePath + "/Figures/zetaValue.png")
        plt.clf()
        ...
        allEpsState.append(copy.deepcopy(recorder.dataRecorded["zeta"]))
    ...
    with open(path + "resultData.pkl", "wb") as f:
        pkl.dump((allValueUsed, allMeanRolloutReward, allStateProportion, allEpisodeLength, allEpsState), f)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-p",
        action="store",
        default="training/D3QNHeuristic/config.yaml",
    )
